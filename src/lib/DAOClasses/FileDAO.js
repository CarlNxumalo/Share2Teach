...
class FileDAO {
    ...
    async getFilesByTag2(tagID){
        const query = `SELECT * 
        FROM dbo.FILES F
        JOIN dbo.FileTag FT ON F.FileID = FT.FileID
        JOIN dbo.Tags T ON T.TagID = FT.TagID
        WHERE T.TagID = @tagID AND F.Status = 'Approved'
        ;
        `
    ...
    async getFilesBySubject2(subjectID){

        let connection;
        try {
            connection = await testConnection(); // Database connection
            const query = `SELECT * FROM FILES WHERE SubjectID = @subjectID AND Status = 'Approved';`
            const result = await connection.request().input('subjectID', sql.Int, subjectID).query(query);
            return result.recordset; // Return the array of files
        } catch (err) {
            console.error('Failed to fetch files by subject ID:', err);
    ...
    async searchDocument2(searchTerm) {
        ...
        try {
            ...
            const query = `
                SELECT f.*
                FROM Files f
                WHERE f.Path LIKE @searchTerm AND f.Status = 'Approved';
            `;
